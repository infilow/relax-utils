...
public final class GenericHelper {
    ...
    public static Type getType(final Field field) {
        return Optional.ofNullable(getTypeParameters(field.getType(), field.getGenericType())).map(ps -> ps[0]).orElse(null);
    }

    public static Type getType(final Parameter parameter) {
        return Optional.ofNullable(getTypeParameters(parameter.getType(), parameter.getGenericType())).map(ps -> ps[0]).orElse(null);
    }

    public static Type getReturnType(final Method method) {
        return Optional.ofNullable(getTypeParameters(method.getReturnType(), method.getGenericReturnType())).map(ps -> ps[0]).orElse(null);
    }
