...
public final class FileHelper {
    ...
    public static File[] ls(String path) {
        ...
        File file = of(path); if (file.isDirectory()) {
    ...
    public static boolean isEmpty(File file) {
        ...
        if (file.isDirectory()) {
            String[] subFiles = file.list(); return Arrays.isEmpty(subFiles);
        } else if (file.isFile()) {
    ...
    public static void walkFiles(File file, Consumer<File> consumer) {
        if (file.isDirectory()) {
            final File[] subFiles = file.listFiles(); if (Arrays.isNotEmpty(subFiles)) {
    ...
    public static List<String> listFileNames(String path) throws IOException {
        ...
        } int index = path.lastIndexOf(JAR_PATH_EXT); if (index < 0) {
            // common dir
            final List<String> paths = new ArrayList<>(); final File[] files = ls(path); for (File file : files) {
                if (file.isFile()) {
        ...
        // jar file path
        index = index + JAR_FILE_EXT.length(); JarFile jarFile = null; try {
            jarFile = new JarFile(path.substring(0, index)); return ZipHelper.listFileNames(jarFile, Strings.removePrefix(path.substring(index + 1), "/"));
        } catch (IOException e) {
    ...
    public static File of(File directory, String... names) {
        Require.checkNotNull(directory, "directory must not be null"); if (Arrays.isEmpty(names)) {
            return directory;
        }
    ...
    public static File of(String... names) {
        ...
        File file = null; for (String name : names) {
            if (file == null) {
                file = of(name);
            } else {
                file = of(file, name);
    ...
    public static boolean exist(String directory, String regexp) {
        final File file = new File(directory); if (!file.exists()) {
            return false;
        }

        final String[] fileList = file.list(); if (fileList == null) {
            return false;
        }
    ...
    public static long size(File file) {
        ...
        if (file.isDirectory()) {
            long size = 0L; File[] subFiles = file.listFiles(); if (Arrays.isEmpty(subFiles)) {
    ...
    public static boolean del(File file) throws IOException {
        ...
        if (file.isDirectory()) {
            boolean isOk = clean(file); if (!isOk) {
                return false;
            }
        }

        final Path path = file.toPath(); try {
    ...
    public static boolean clean(File directory) throws IOException {
        ...
        final File[] files = directory.listFiles(); if (null != files) {
    ...
    public static boolean cleanEmpty(File directory) {
        ...
        final File[] files = directory.listFiles(); if (Arrays.isEmpty(files)) {
    ...
    public static File createTempFile(String prefix, String suffix, File dir, boolean isReCreat) throws IOException {
        int exceptionsCount = 0; while (true) {
            try {
                File file = File.createTempFile(prefix, suffix, mkdir(dir)).getCanonicalFile(); if (isReCreat) {
                    // noinspection ResultOfMethodCallIgnored
                    file.delete();
                    // noinspection ResultOfMethodCallIgnored
                    file.createNewFile();
    ...
    public static File copyFile(String src, String dest, StandardCopyOption... options) throws IOException {
        Require.checkNotNull(src, "Source File path is blank !"); Require.checkNotNull(dest, "Destination File path is blank !");
    ...
    public static File copyFile(File src, File dest, StandardCopyOption... options) throws IOException {
        // check
        Require.checkNotNull(src, "Source File is null !"); if (!src.exists()) {
            throw new IOException("File not exist: " + src);
        } Require.checkNotNull(dest, "Destination File or directiory is null !"); if (src.equals(dest)) {
    ...
    public static void move(File src, File target, boolean isOverride) throws IOException {
        Require.checkNotNull(src, "Src file must be not null!"); Require.checkNotNull(target, "target file must be not null!");
    ...
    public static File rename(File file, String newName, boolean isRetainExt, boolean isOverride) throws IOException {
        if (isRetainExt) {
            final String extName = getExtensionName(file); if (Strings.isNotBlank(extName)) {
    ...
    public static String getExtensionName(String fileName) {
        if (fileName == null) {
            return null;
        } int index = fileName.lastIndexOf("."); if (index == -1) {
            return "";
        } else {
            String ext = fileName.substring(index + 1); return ext.contains("/") || ext.contains("\\") ? "" : ext;
        }
    }
    ...
    public static String getAbsolutePath(String path) {
        ...
        File file = Paths.get(path).toFile(); try {
            return file.getCanonicalPath();
        } catch (IOException e) {
            return file.getAbsolutePath();
        }
    ...
    public static boolean equals(File file1, File file2) throws IOException {
        Require.checkNotNull(file1); Require.checkNotNull(file2); if (!file1.exists() || !file2.exists()) {
    ...
    public static int lastIndexOfSeparator(String filePath) {
        if (Strings.isNotEmpty(filePath)) {
            int i = filePath.length(); char c; while (--i >= 0) {
                c = filePath.charAt(i); if (Chars.isFileSeparator(c)) {
                    return i;
                }
            }
        } return -1;
    ...
    public static String readString(URL url, Charset charset) {
        ...
        InputStream in = null; try {
            in = url.openStream(); return IOStreams.read(in, charset);
        } catch (IOException e) {
    ...
    public static <T extends Collection<String>> T readLines(URL url, Charset charset, T collection) throws IOException {
        InputStream in = null; try {
            in = url.openStream(); return IOStreams.readLines(in, charset, collection);
        } catch (IOException e) {
            throw new IOException(e);
        } finally {
            IOStreams.closeQuietly(in);
    ...
    public static void readLine(RandomAccessFile file, Charset charset, Consumer<String> lineHandler) throws IOException {
        final String line = readLine(file, charset); if (null != line) {
            lineHandler.accept(line);
        }
    }

    public static String readLine(RandomAccessFile file, Charset charset) throws IOException {
        String line = file.readLine(); if (null != line) {
    ...
    public static String getParent(String filePath, int level) throws IOException {
        final File parent = getParent(of(filePath), level); return null == parent ? null : parent.getCanonicalPath();
    }
    ...
    public static File getParent(File file, int level) throws IOException {
        ...
        File parentFile = file.getCanonicalFile().getParentFile(); if (1 == level) {
            return parentFile;
    ...
    public static File checkSlip(File parentFile, File file) throws IllegalArgumentException {
        if (null != parentFile && null != file) {
            String parentCanonicalPath; String canonicalPath; try {
                parentCanonicalPath = parentFile.getCanonicalPath(); canonicalPath = file.getCanonicalPath();
            } catch (IOException e) {
                parentCanonicalPath = parentFile.getAbsolutePath(); canonicalPath = file.getAbsolutePath();
    ...
    public static String getMimeType(String filePath) throws IOException {
        String contentType = URLConnection.getFileNameMap().getContentTypeFor(filePath); if (null == contentType) {
    ...
    public static boolean isChild(File parent, File child) {
        Require.checkNotNull(parent); Require.checkNotNull(child); return PathHelper.isChild(parent.toPath(), child.toPath());
    }
    ...
    private static File buildFile(File outFile, String fileName) {
        fileName = fileName.replace('\\', '/'); if (!isWindows() && fileName.lastIndexOf("/", fileName.length() - 2) > 0) {
            final List<String> pathParts = java.util.Arrays.asList(Strings.split(fileName, "/")); final int lastPartIndex = pathParts.size() - 1; for (int i = 0; i < lastPartIndex; i++) {
                outFile = new File(outFile, pathParts.get(i));
            }
            // noinspection ResultOfMethodCallIgnored
            outFile.mkdirs(); fileName = pathParts.get(lastPartIndex);
        }
